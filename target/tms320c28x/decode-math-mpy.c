...
static void gen_mac_p_loc16_pma(DisasContext *ctx, uint32_t mode, uint32_t addr)
{
    ...
    tcg_gen_mul_i32(cpu_p, v1, v2);//P = signed T * signed Prog[0x00:pma]
...
//QMPYSL P,XT,loc32
static void gen_qmpysl_p_xt_loc32(DisasContext *ctx, uint32_t mode)
{
    TCGv a = tcg_temp_local_new();
    TCGv b = tcg_temp_local_new();
    TCGv v2 = tcg_temp_local_new();
...
static void gen_sqra_loc16(DisasContext *ctx, uint32_t mode)
{
    ...
    tcg_gen_add_i32(cpu_acc, tmp, tmp2);
    gen_helper_test_C_V_32(cpu_env, tmp, tmp2, cpu_acc);
    gen_helper_test_N_Z_32(cpu_env, cpu_acc);
    gen_helper_test_OVC_OVM_32(cpu_env, tmp, tmp2, cpu_acc);
...
//XMAC P,loc16,*(pma)
static void gen_xmac_p_loc16_pma(DisasContext *ctx, uint32_t mode, uint32_t pma)
{
    TCGLabel *begin = gen_new_label();
    TCGLabel *end = gen_new_label();
    TCGv v1 = tcg_temp_local_new();
